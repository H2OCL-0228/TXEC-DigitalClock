...
subject.switch = function (type) {
  ...
  slogan.$main = exams[type].mainSlogan || "文明二初&emsp;从我做起";
...
function setTemp(ts, sh, sm, eh, em) {
  ...
  if (!(ts = prompt("晚自习科目名称(3个字以内)", "考练")) ||
    !(sh = prompt("晚自习开始时间(小时)", 16)) ||
    !(sm = prompt("晚自习开始时间(分钟)", 25)) ||
    !(eh = prompt("晚自习结束时间(小时)", 23)) ||
    !(em = prompt("晚自习结束时间(分钟)", 55)))
    // 取消创建临时科目
